/*CMNDRepqueryRTNode.cpp
  this file implements class CMNDRepqueryRTNode*/
...
//------------------------------------------------------------
CMNDRepqueryRTNode::CMNDRepqueryRTNode(CMNDRepqueryRTree *rt)
  //use this ructor to create a new node on disk.
{
    ...
    entries = new CMNDRepqueryEntry[capacity];
...
//------------------------------------------------------------
CMNDRepqueryRTNode::CMNDRepqueryRTNode(CMNDRepqueryRTree *rt, int _block)
...
//------------------------------------------------------------
CMNDRepqueryRTNode::~CMNDRepqueryRTNode()
{
    char *b;
...
//------------------------------------------------------------
int CMNDRepqueryRTNode::choose_subtree(float *mbr)
{
    ...
    // else choose the one for which insertion causes the minimun area enlargement
    {
       	if (level == 1) // son_is_data
    	{
    	    ...
    	    for (i = 0; i < num_entries; i++)
    	    {
    	        ...
    	        // is this CMNDRepqueryEntry better than the former optimum ?
    	        if ((o < omin) ||
    		    (o == omin && f < fmin) ||
...
//------------------------------------------------------------
R_DELETE CMNDRepqueryRTNode::delete_entry(CMNDRepqueryEntry *e)
{
	CMNDRepqueryRTNode *succ;
	float *tmp;
	if (level > 0)
	{
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::enter(CMNDRepqueryEntry *de)
  //note that de will be deleted after being entered.
{
    if (num_entries > (capacity-1))
        error("CMNDRepqueryRTNode::enter: called, but node is full", TRUE);
...
//------------------------------------------------------------
bool CMNDRepqueryRTNode::FindLeaf(CMNDRepqueryEntry *e)
{
	CMNDRepqueryRTNode *succ;
	if (level > 0)
	{
...
//------------------------------------------------------------
float* CMNDRepqueryRTNode::get_mbr()
{
    int i, j;
    float *mbr;
...
//------------------------------------------------------------
int CMNDRepqueryRTNode::get_num_of_data()
{
    int i, sum;
    CMNDRepqueryRTNode* succ;
...
//------------------------------------------------------------
R_OVERFLOW CMNDRepqueryRTNode::insert(CMNDRepqueryEntry *d, CMNDRepqueryRTNode **sn)
{
    int follow;
    CMNDRepqueryRTNode *succ, *new_succ;
    CMNDRepqueryRTNode *brother;
	  ...
	  if (level > d -> level)
	  {
        ...
        // node has split into itself and *new_succ
        {
            if (num_entries == capacity)
         	    error("CMNDRepqueryRTNode::insert: maximum capacity violation", TRUE);

            de = new CMNDRepqueryEntry(dimension, my_tree);
            ...
            if (num_entries == (capacity - 1))
            {
        	    brother = new CMNDRepqueryRTNode(my_tree);
        	    my_tree -> num_of_inodes++;
        	    brother -> level = level;
        	    split(brother);
                *sn = brother;
    ...
    else // data (leaf) node
    {
        ...
        // for the algorithms are more easy then
        {
			...
			//-----------By TAO Yufei
            {
                ...
                new_entries = new CMNDRepqueryEntry[capacity];
           	...
           	{
        	    *sn = new CMNDRepqueryRTNode(my_tree);
        	    (*sn) -> level = level;
        	    my_tree -> num_of_dnodes++;
        	    split((CMNDRepqueryRTNode *) *sn);
    	    }
    	    return SPLIT;
        }
        else
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::rangeQuery(float *mbr, SortedLinList *res)
{
    int i, n;
    SECTION s;
    CMNDRepqueryRTNode *succ;
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::read_from_buffer(char *buffer)
{
    int i, j, s;
...
//------------------------------------------------------------
int CMNDRepqueryRTNode::split(float **mbr, int **distribution)
{
    bool lu;
	...
	// the array distribution is deleted in split(CMNDRepqueryRTNode *sn);
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::split(CMNDRepqueryRTNode *sn)
{
    int i, *distribution, dist, n;
    float **mbr_array;
    CMNDRepqueryEntry *new_entries1, *new_entries2;
    ...
    new_entries1 = new CMNDRepqueryEntry[capacity];
    new_entries2 = new CMNDRepqueryEntry[capacity];
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::write_to_buffer(char *buffer)
{
    int i, j, s;
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::rect_win_query(float *mbr, LinList *in_objs, LinList *out_objs_so_far)
{
    int i, n;
    SECTION s;
    CMNDRepqueryRTNode *succ;
...
//------------------------------------------------------------
void CMNDRepqueryRTNode::rect_win_query(float *mbr, float *exclmbr, LinList *c_inf_objs)
{
    int i, n;
    SECTION s, s1;
    CMNDRepqueryRTNode *succ;
