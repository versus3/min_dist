/*CMNDRTNode.cpp
  this file implements class CMNDRTNode*/
...
//------------------------------------------------------------
CMNDRTNode::CMNDRTNode(CMNDRTree *rt)
  //use this ructor to create a new node on disk.
{
    ...
    entries = new CMNDEntry[capacity];
...
//------------------------------------------------------------
CMNDRTNode::CMNDRTNode(CMNDRTree *rt, int _block)
...
//------------------------------------------------------------
int CMNDRTNode::choose_subtree(float *mbr)
{
    ...
    // else choose the one for which insertion causes the minimun area enlargement
    {
       	if (level == 1) // son_is_data
    	{
    	    ...
    	    for (i = 0; i < num_entries; i++)
    	    {
        		...
        		// calculate overlap before enlarging CEntry_i
        		old_o = o = 0.0;
    	        ...
    	        // is this CMNDEntry better than the former optimum ?
    	        if ((o < omin) ||
    		    (o == omin && f < fmin) ||
...
//------------------------------------------------------------
R_DELETE CMNDRTNode::delete_entry(CMNDEntry *e)
{
	CMNDRTNode *succ;
	float *tmp;
	if (level > 0)
...
//------------------------------------------------------------
void CMNDRTNode::enter(CMNDEntry *de)
  //note that de will be deleted after being entered.
{
    if (num_entries > (capacity-1))
        error("CMNDRTNode::enter: called, but node is full", TRUE);
...
//------------------------------------------------------------
bool CMNDRTNode::FindLeaf(CMNDEntry *e)
{
	CMNDRTNode *succ;
	if (level > 0)
	{
...
//------------------------------------------------------------
int CMNDRTNode::get_num_of_data()
{
    int i, sum;
    CMNDRTNode* succ;
...
//------------------------------------------------------------
R_OVERFLOW CMNDRTNode::insert(CMNDEntry *d, CMNDRTNode **sn)
{
    int follow;
    CMNDRTNode *succ, *new_succ;
    CMNDRTNode *brother;
    CMNDEntry *de;
    R_OVERFLOW ret;
    float *mbr,*nmbr;
	  ...
	  if (level > d -> level)
	  {
        ...
        // node has split into itself and *new_succ
        {
            if (num_entries == capacity)
         	    error("CMNDRTNode::insert: maximum capacity violation", TRUE);

            de = new CMNDEntry(dimension, my_tree);
            ...
            if (num_entries == (capacity - 1))
            {
        	    brother = new CMNDRTNode(my_tree);
        	    my_tree -> num_of_inodes++;
        	    brother -> level = level;
        	    split(brother);
                *sn = brother;
    ...
    else // data (leaf) node
    {
        ...
        // for the algorithms are more easy then
        {
			...
			//-----------By TAO Yufei
            {
                ...
                new_entries = new CMNDEntry[capacity];
...
//------------------------------------------------------------
void CMNDRTNode::rangeQuery(float *mbr, SortedLinList *res)
{
    int i, n;
    SECTION s;
    CMNDRTNode *succ;
...
//------------------------------------------------------------
void CMNDRTNode::read_from_buffer(char *buffer)
{
    int i, j, s;
...
//------------------------------------------------------------
int CMNDRTNode::split(float **mbr, int **distribution)
{
    bool lu;
	...
	// the array distribution is deleted in split(CMNDRTNode *sn);
...
//------------------------------------------------------------
void CMNDRTNode::split(CMNDRTNode *sn)
{
    int i, *distribution, dist, n;
    float **mbr_array;
    CMNDEntry *new_entries1, *new_entries2;
    ...
    new_entries1 = new CMNDEntry[capacity];
    new_entries2 = new CMNDEntry[capacity];
...
//------------------------------------------------------------
void CMNDRTNode::write_to_buffer(char *buffer)
{
    int i, j, s;
...
//------------------------------------------------------------
void CMNDRTNode::rect_win_query(float *mbr, LinList *in_objs, LinList *out_objs_so_far)
{
    int i, n;
    SECTION s;
    CMNDRTNode *succ;
...
//------------------------------------------------------------
void CMNDRTNode::rect_win_query(float *mbr, float *exclmbr, LinList *c_inf_objs)
{
    int i, n;
    SECTION s, s1;
    CMNDRTNode *succ;
