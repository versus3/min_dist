...
/*CBBRTree.cpp
  this file implements the CBBRTree class*/
#include <math.h>
#include <string.h>
#include "bbrtree.h"
#include "bbentry.h"
...
//------------------------------------------------------------
CBBRTree::CBBRTree(char *fname, int _b_length, Cache *c, int _dimension)
  //use this constructor to build a new tree
{
    ...
    root_ptr = new CBBRTNode(this);
	  //note that when a tree is constructed, the root is automatically created
	  //though at this time there is no CBBEntry in the root yet.
    num_of_dnodes++;
    root_ptr -> level = 0;
    root = root_ptr -> block;
...
//------------------------------------------------------------
CBBRTree::CBBRTree(char *fname, Cache *c)
...
//------------------------------------------------------------
CBBRTree::CBBRTree(char *inpname, char *fname, int _b_length, Cache *c, int _dimension)
...
//------------------------------------------------------------
void CBBRTree::del_root()
{
	delete root_ptr;
	root_ptr = NULL;
}
//------------------------------------------------------------
bool CBBRTree::delete_entry(CBBEntry *d)
{
	load_root();
	...
	if (del_ret == ERASED) 
		error("CBBRTree::delete_entry--The root has been deleted\n",true);
 
	if (root_ptr -> level > 0 && root_ptr -> num_entries == 1)
		//there is only one CBBEntry in the root but the root
	...
	while (deletelist -> get_num() > 0)
	{
		...
		CBBEntry *new_e = new CBBEntry(dimension, NULL);
...
//------------------------------------------------------------
void CBBRTree::insert(CBBEntry* d)
{
    int i, j;
    CBBRTNode *sn;
    CBBRTNode *nroot_ptr;
    ...
    // insert d into re_data_cands as the first CBBEntry to insert
    // make a copy of d because it should be erased later
    Linkable *new_link;
	new_link = d -> gen_Linkable();
	re_data_cands -> insert(new_link);

	delete d;  //we follow the convention that the CBBEntry will be deleted when insertion finishes
    ...
    while (re_data_cands -> get_num() > 0)
    {
        ...
        if (d_cand != NULL)
        {
            ...
            // erasing it
			dc = new CBBEntry(dimension, NULL);
...
//------------------------------------------------------------
void CBBRTree::load_root()
{
    if (root_ptr == NULL)
        root_ptr = new CBBRTNode(this, root);
}
//------------------------------------------------------------
void CBBRTree::rangeQuery(float *mbr, SortedLinList *res)
{
    load_root();
...
//------------------------------------------------------------
void CBBRTree::read_header(char *buffer)
{
    int i;
...
//------------------------------------------------------------
int CBBRTree::update_rslt(CBBEntry *_e, float _dist, CBBEntry *_rslt, 
						float *_key, int _k)
{
	for (int i = 0; i < _k; i ++)
	{
...
//------------------------------------------------------------
void CBBRTree::write_header(char *buffer)
{
    int i;
...
void CBBRTree::rect_win_query(float *mbr, LinList *in_objs, LinList *out_objs_so_far)
{
    load_root();
...
void CBBRTree::rect_win_query(float *mbr, float *exclmbr, LinList *c_inf_objs)
{
    load_root();
...
void CBBRTree::BFNN(float *_qpt, int _k, CBBEntry *_rslt)
{
	...
	int son = root; //this CBBEntry is to be visited next
	while (son != -1)
	{
		CBBRTNode *rtn = new CBBRTNode(this, son);
		...
		//get next CBBEntry from the heap----------------------------
		HeapEntry *he = new HeapEntry();
		bool again = true;
		while (again)
		{
...
void CBBRTree::TPNN_TP(float *_qline, int _k, CBBEntry *_nn, CBBEntry *_rslt, float _max_trvl)
{
	float key = _max_trvl; 
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^leaf node case^^^^^^^^^^^^^^^^^^^^^
			{	
				...
				if (eNNsub==-1 || SEQUENCE_SENSITIVE)
					...
					//then we do not handle this CBBEntry
				{
...
/*
							...
							//(for CBBEntry e) with respect to(nn[j])
