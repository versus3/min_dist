...
/*bbrepquerytree.cpp
  this file implements the CMNDRepqueryRTree class*/
#include <math.h>
#include <string.h>
#include "mndrepqueryrtree.h"
...
//------------------------------------------------------------
CMNDRepqueryRTree::CMNDRepqueryRTree()
{
}
//------------------------------------------------------------
CMNDRepqueryRTree::CMNDRepqueryRTree(char *fname, int _b_length, Cache *c, int _dimension)
  //use this constructor to build a new tree
{
    ...
    root_ptr = new CMNDRepqueryRTNode(this);
	  //note that when a tree is constructed, the root is automatically created
	  //though at this time there is no CMNDRepqueryEntry in the root yet.
    num_of_dnodes++;
    root_ptr -> level = 0;
    root = root_ptr -> block;
...
//------------------------------------------------------------
CMNDRepqueryRTree::CMNDRepqueryRTree(char *fname, Cache *c)
...
//------------------------------------------------------------
CMNDRepqueryRTree::CMNDRepqueryRTree(char *inpname, char *fname, int _b_length, Cache *c, int _dimension)
  // construct new R-tree from a specified input textfile with rectangles
{
    ...
    else
    {
      while (!feof(fp))
      {
		...
		d = new CMNDRepqueryEntry(dimension, NULL);
...
//------------------------------------------------------------
void CMNDRepqueryRTree::del_root()
{
	delete root_ptr;
	root_ptr = NULL;
}
//------------------------------------------------------------
bool CMNDRepqueryRTree::delete_entry(CMNDRepqueryEntry *d)
{
	load_root();
	...
	if (del_ret == ERASED) 
		error("CMNDRepqueryRTree::delete_entry--The root has been deleted\n",true);
 
	if (root_ptr -> level > 0 && root_ptr -> num_entries == 1)
		//there is only one CMNDRepqueryEntry in the root but the root
	...
	while (deletelist -> get_num() > 0)
	{
		...
		CMNDRepqueryEntry *new_e = new CMNDRepqueryEntry(dimension, NULL);
...
//------------------------------------------------------------
void CMNDRepqueryRTree::insert(CMNDRepqueryEntry* d)
{
    int i, j;
    CMNDRepqueryRTNode *sn;
    CMNDRepqueryRTNode *nroot_ptr;
    int nroot;
    CMNDRepqueryEntry *de;
    R_OVERFLOW split_root;
    CMNDRepqueryEntry *dc;
    float *nmbr;
    ...
    // insert d into re_data_cands as the first CMNDRepqueryEntry to insert
    // make a copy of d because it should be erased later
    Linkable *new_link;
	new_link = d -> gen_Linkable();
	re_data_cands -> insert(new_link);

	delete d;  //we follow the convention that the CMNDRepqueryEntry will be deleted when insertion finishes
    ...
    while (re_data_cands -> get_num() > 0)
    {
        ...
        if (d_cand != NULL)
        {
            ...
            // erasing it
			dc = new CMNDRepqueryEntry(dimension, NULL);
	        ...
	        error("CMNDRepqueryRTree::insert: inconsistent list re_data_cands", TRUE);

    	if (split_root == SPLIT)
    	// insert has lead to split --> new root-page with two sons (i.e. root and sn)
    	{
    	    nroot_ptr = new CMNDRepqueryRTNode(this);
    	    nroot_ptr -> level = root_ptr -> level + 1;
    	    num_of_inodes++;
    	    nroot = nroot_ptr -> block;

    	    de = new CMNDRepqueryEntry(dimension, this);
...
//------------------------------------------------------------
void CMNDRepqueryRTree::load_root()
{
    if (root_ptr == NULL)
        root_ptr = new CMNDRepqueryRTNode(this, root);
}
//------------------------------------------------------------
void CMNDRepqueryRTree::rangeQuery(float *mbr, SortedLinList *res)
{
    load_root();
...
//------------------------------------------------------------
void CMNDRepqueryRTree::read_header(char *buffer)
{
    int i;
...
//------------------------------------------------------------
int CMNDRepqueryRTree::update_rslt(CMNDRepqueryEntry *_e, float _dist, CMNDRepqueryEntry *_rslt, 
						float *_key, int _k)
{
	for (int i = 0; i < _k; i ++)
	{
...
//------------------------------------------------------------
void CMNDRepqueryRTree::write_header(char *buffer)
{
    int i;
...
void CMNDRepqueryRTree::rect_win_query(float *mbr, LinList *in_objs, LinList *out_objs_so_far)
{
    load_root();
...
void CMNDRepqueryRTree::rect_win_query(float *mbr, float *exclmbr, LinList *c_inf_objs)
{
    load_root();
...
void CMNDRepqueryRTree::BFNN(float *_qpt, int _k, CMNDRepqueryEntry *_rslt)
{
	...
	int son = root; //this CMNDRepqueryEntry is to be visited next
	while (son != -1)
	{
		CMNDRepqueryRTNode *rtn = new CMNDRepqueryRTNode(this, son);
		...
		//get next CMNDRepqueryEntry from the heap----------------------------
		HeapEntry *he = new HeapEntry();
		bool again = true;
		while (again)
		{
...
void CMNDRepqueryRTree::TPNN_TP(float *_qline, int _k, CMNDRepqueryEntry *_nn, CMNDRepqueryEntry *_rslt, float _max_trvl)
{
	float key = _max_trvl; 
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^leaf node case^^^^^^^^^^^^^^^^^^^^^
			{	
				...
				if (eNNsub==-1 || SEQUENCE_SENSITIVE)
					...
					//then we do not handle this CMNDRepqueryEntry
				{
...
/*
							...
							//(for CMNDRepqueryEntry e) with respect to(nn[j])
